...
import การเดินทาง from './svg/การเดินทาง.svg';
import กิจกรรม from './svg/กิจกรรม.svg';
import ที่พัก from './svg/ที่พัก.svg';
import อาหาร from './svg/อาหาร.svg';
import อื่นๆ from './svg/อื่นๆ.svg';
...
function Home() {
    ...
    React.useEffect(() => {
        ...
        fetch("/api/category")
            ...
            .then((Categories) => {
                ...
                document.getElementById('select-category').innerHTML = '<option>Select Category</option>';
    ...
    function updateSearchIcon() {
        ...
        document.getElementById('CategoryIcon').setAttribute('src', `%PUBLIC_URL%/svg/${selected}.svg`);
    }
                ...
                <div className="col-start-1 col-end-7 flex justify-center px-4">
                    <div className="grid grid-cols-3 gap-6 xl:w-50vw w-full">
                        <div className="col-start-1 relative inline-flex self-center w-min justify-self-end">
                            <img id="CategoryIcon" className="md:inline hidden" alt="React Logo" />
                            ...
                            <select onChange={updateSearchIcon} id="select-category" className="link w-min hover:cursor-pointer text-black h-10 bg-transparent pl-5 pr-8 focus:outline-none appearance-none">
